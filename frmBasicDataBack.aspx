...
Class Default2
    ...
    Function GetManData(ByVal sortBy As String) As System.Data.OleDb.OleDbDataReader
        ...
        Select Case DropDownList1.SelectedIndex
            Case Is = 1
                sqlString = " SELECT * FROM masrofat where date >= txtSearch.Text.ToString and date <= txtSearchD.Text.ToString" & " ORDER By " & sortBy
                '  sqlString = " SELECT * FROM masrofat where date >= ? and date <= @ ORDER By " & sortBy
                txtBand = " التاريخ من = " & txtSearch.Text.ToString & " إلى " & txtSearchD.Text.ToString
                '       s1 = "date=" & txtSearch.Text.ToString()
            Case Is = 2
                sqlString = " SELECT * FROM masrofat where kima = txtSearch.Text.ToString"  '1000'' & txtSearch.Text.ToString & " ORDER By " & sortBy '?   ORDER By " & sortBy
                txtBand = " القيمة = " & txtSearch.Text.ToString
                '       s1 = "kima=" & txtSearch.Text.ToString
            Case Is = 3
                sqlString = " SELECT * FROM masrofat where desc = ?   ORDER By " & sortBy
            ...
            Case Is = 4
                ...
                txtBand = " البند = " & txtSearch.Text.ToString
                '      s1 = "item=" & txtSearch.Text.ToString
            Case Is = 5
                sqlString = " SELECT * FROM masrofat where subitem=?   ORDER By " & sortBy
                txtBand = " البند الفرعى = " & txtSearch.Text.ToString
                '     s1 = "subitem=" & txtSearch.Text.ToString
            Case Is = 6
                sqlString = " SELECT * FROM masrofat where operation =" & txtSearch.Text.ToString & " ORDER By " & sortBy
            ...
            Case Is = 7
                ...
                txtBand = " مسئول الصرف = " & txtSearch.Text.ToString
        ...
        '  If MsgBox("هل ترغب فى خذف البند؟", 528387, "رسالة تأكيد") = 6 Then

        '  End If

        If dataReader.HasRows = False Then
            Response.Write(MsgBox("No Record Found !", 65536, "  "))
    ...
    Protected Sub GridView1_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles GridView1.RowDeleting
        ...
        If MsgBox("هل ترغب فى خذف البند؟", 1576963, "رسالة تأكيد") = 6 Then ' 528387
            ...
            Dim sqlString As String = ""
            sqlString = " DELETE * FROM masrofat  WHERE ID  =" & gcount
            Dim sqlCommand As New OleDbCommand(sqlString, sqlConnection)
            sqlConnection.Open()
    ...
    End Sub
    Protected Sub txtSearch_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        If e.KeyCode = System.Windows.Forms.Keys.F2 Then
            lblRCount.Text = "aaaaaa"
        End If
    End Sub
    ...
    Protected Sub ButtonGroup_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonGroup.Click
        ...
        Select Case DropDownList1.SelectedIndex
            Case Is = 3
                sqlString = " SELECT count(ID), desc FROM masrofat where desc like ? GROUP BY " & " desc" & " order by " & " desc"
                DropDownListGroup.DataTextField = "desc"
                DropDownListGroup.DataValueField = "desc"
            Case Is = 4
                sqlString = " SELECT  count(ID) ,  item FROM masrofat where item like? GROUP BY " & "item" & " order by " & " item"
                DropDownListGroup.DataTextField = "item"
                DropDownListGroup.DataValueField = "item"
            Case Is = 5
                sqlString = " SELECT  count(ID) ,  subitem FROM masrofat where subitem like? GROUP BY " & "subitem" & " order by " & " subitem"
                DropDownListGroup.DataTextField = "subitem"
                DropDownListGroup.DataValueField = "subitem"
            Case Is = 6
            ...
            Case Is = 7
                sqlString = " SELECT  count(ID) ,  sarf FROM masrofat where sarf like? GROUP BY " & "sarf" & " order by " & " sarf"
                DropDownListGroup.DataTextField = "sarf"
                DropDownListGroup.DataValueField = "sarf"
            Case Is = 8
                sqlString = " SELECT  count(ID) ,  notes FROM masrofat where notes like? GROUP BY " & "notes" & " order by " & " notes"
